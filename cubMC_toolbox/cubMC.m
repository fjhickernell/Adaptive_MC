function [Q,param]=cubMC(f,interval,param)
...
if strcmp(param.sample,'iid') %iid sampling
    ...
    if sig0up==0;
        ...
        if NBEfun(log(sqrt(param.n0))) <= 0;
            param.n=param.n0;
        else
            logsqrtnCLT=log(norminv(1-alpha1/2)*sig0up/param.tol);
            param.n=ceil(exp(2*fzero(NBEfun,logsqrtnCLT)));
        end
    end
