function [interval,param]=quadMCparam(interval,param,whfield)
%   This function checks the inputs of quadMCparam to insure that 
...
%Default values
 measureDefault='uniform'; %distribution to integrate against
     tolDefault=1e-2;      %absolute error tolerance
 ...
 ndpcmaxDefault=1e5;       %maximum size of a vector of samples
      n0Default=1024;      %initial number of samples
   n0MinDefault=30;        %minimum allowed initial number of samples
   alphaDefault=0.05;      %uncertainty
...
npartMaxDefault=32;        %maximum allowed
   fudgeDefault=1.5;       %fudge factor for error estimate
...
if any(interval(1,:)==interval(2,:)); %interval is a point in one direction
    param.exit=12; param=quadMCerr(param); return;
end
...
if isfield(param,'impyes');
    if isfield(param,'impscale');
        if any(param.impscale<=0); %scale must be positive 
            param.exit=20; param=quadMCerr(param); return;
        end
