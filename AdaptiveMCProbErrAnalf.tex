...
\begin{equation} \label{GaussTestFun}
f(\vx)=a_0 + b_0\prod_{j=1}^d\left[ 1 +b_j \exp \left(-\frac{(x_j-h_j)^2}{c_j^2}\right) \right].
\end{equation}
Here $\vx$ is a $d$ dimensional vector, and the $b_j$, $c_j$, and $h_j$ are parameters.  For Figure \ref{GaussTestFunFig} shows the results of different algorithms being used to integrate $500$ different instances of $f$ being are performed.  For each replication, the parameters determining $f$ are chosen as follows:
\begin{itemize} 
